...
async function completeChap(popup, pf, ci) {
  ...
  // Get server URL and duration from iframe
  var serverUrl = 'https://content.hst360.com';
  var duration = 3600;
  
  // Step 1: Wait for DPlayer to make its OWN first real saveProgress
  // Intercept response to get updated token
  var validatedToken = null;
  
  popup.route('**/takeRecordByToken**', async function(route, request) {
    // Let the request go through to the server naturally
    // Capture the response to get the new token
    ...
    try {
      ...
      if (j.data && typeof j.data === 'string' && j.data.length > 20) {
        ...
        log('Got validated token from real save');
      }
    } catch(e) {}
  });
  
  // Wait up to 40s for first save (DPlayer saves every ~28s)
  for (var w = 0; w < 40; w++) {
    if (validatedToken) break;
    // Also extract URL/duration
  ...
  if (!validatedToken) {
    // Fallback: extract token from iframe directly
  ...
  if (!validatedToken) { log('NO TOKEN'); return false; }
  log('Token: ' + validatedToken.substring(0, 16) + '... dur=' + duration + ' url=' + serverUrl);
  
  // Step 2: Now make the isEnd=true call ourselves with the validated token
  var result = await pf.evaluate(function(o) {
    ...
    function toB60(s) {
      ...
      if (n === 0n) return '0';
      var r = ''; while (n > 0n) { r = ALPH[Number(n % 60n)] + r; n = n / 60n; } return r;
    }
    ...
    .then(function(sig) {
      ...
      }).then(function(r) { return r.text().then(function(t) { return 'st='+r.status+' r='+t.substring(0,120); }); })
      .catch(function(e) { return 'fetch:'+e.message; });
    }).catch(function(e) { return 'hmac:'+e.message; });
  }, {t: validatedToken, d: duration, srv: serverUrl});
  
  log('API: ' + result);
  return result.indexOf('code=0') >= 0 || result.indexOf('"code":0') >= 0 || result.indexOf('"code":"0"') >= 0;
}

(async function() {
  fs.writeFileSync(LF,''); log('=== DONE v11 (validated token) ===');
  ...
  for(var ci=0;ci<courses.length;ci++){
    ...
    for(var ci2=0;ci2<chapters.length;ci2++){
      // Register popup handler BEFORE clicking
