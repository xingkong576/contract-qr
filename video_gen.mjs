 ...
 * 
 * Usage:
 *   node video_gen.mjs "prompt" "image_path" [output.mp4]
 *
 * Examples:
 *   node video_gen.mjs "A black cat blinking slowly" C:\path\to\cat.jpg cat_video.mp4
 */
...
const API_ENDPOINT = 'https://apihub.agnes-ai.com/v1/videos';
        ...
        res.on('end', () => {
          try {
            ...
            if (j.status === 'completed' && j.data && j.data.url) {
              ...
              reject(new Error(`Failed: ${j.error || JSON.stringify(j)}`));
            } else {
              retries++;
              if (retries >= maxRetries) {
                reject(new Error('Timeout: max retries exceeded'));
              } else {
...
async function main() {
  ...
  if (!prompt || !imagePath) {
    console.error('Usage: node video_gen.mjs "prompt" "image_path" [output.mp4]');
    process.exit(1);
  }
