...
MAX_POSITIONS = 5
MIN_ATR_STOP = 0.08          # ATR动态止损基础8%
MAX_ATR_STOP = 0.15          # ATR动态止损上限15%
TRAILING_STOP_PCT = 0.05     # 移动止盈（最高点回撤5%）
MAX_HOLD_DAYS = 30           # 最长持有30天
MIN_STRENGTH = 6             # 最小信号强度
COMMISSION = 0.0003
SECTOR_LIMIT = 2
...
def run_backtest():
    ...
    for date in trading_dates:
        ...
        for i, pos in enumerate(portfolio):
            ...
            elif current_price < pos["highest"] * (1 - TRAILING_STOP_PCT):
        ...
        for i in sorted(to_remove, reverse=True):
            ...
            if code and code in SECTOR_MAP:
                sector_counts[SECTOR_MAP[code]] = max(0, sector_counts.get(SECTOR_MAP[code], 1) - 1)
        ...
        for cand in candidates:
            ...
            atr_stop = max(MIN_ATR_STOP, min(MAX_ATR_STOP, cand["atr"] / cand["price"]))
            
            portfolio.append({
                "code": cand["code"], "name": cand["name"],
                "buy_price": cand["price"], "buy_date": date,
                "shares": buy_amount, "hold_days": 0,
                "sector": cand["sector"], "atr_stop": atr_stop,
                "highest": cand["price"]  # 追踪最高价
            })
            ...
            print(f"  买入: {cand['name']} {cand['price']:.2f} 强度{cand['strength']} ATR止损{atr_stop*100:.1f}%")
    ...
    pd.DataFrame(daily_values).to_csv(f"backtest_trend_v5_{datetime.now().strftime('%Y%m%d')}.csv", index=False)
    if trades:
        pd.DataFrame(trades).to_csv(f"backtest_trades_v5_{datetime.now().strftime('%Y%m%d')}.csv", index=False)
    print(f"\n文件: backtest_trend_v5_{datetime.now().strftime('%Y%m%d')}.csv")
